...
% Parameters
nTrials = 200;
uniqStimVals = linspace(80, 100, 101); % Unique set of orientations to show
...
for idx=1:numel(sigma_m)
    ...
    sampledStimVal = uniqStimVals' + sigma_d.*randn(numel(uniqStimVals), nTrials);
    ...
    mu_log = log(sigma_d.^2 ./ sqrt(sigma_m_^2 + sigma_d.^2));
    sigma_log = sqrt(log(1 + (sigma_m_^2 ./ sigma_d.^2)));
    sigma_hat = lognrnd(mu_log, sigma_log, [numel(uniqStimVals), nTrials]);
...
for idx=1:numel(sigma_d)
    ...
    % Psychometric - behavioral performance
    sampledStimVal = uniqStimVals' + sigma_d_.*randn(numel(uniqStimVals), nTrials);
    choices = zeros(numel(uniqStimVals), nTrials); 
    choices(sampledStimVal > Cd) = 1; % CCW choices
    ...
    % Confidence computation
    mu_log = log(sigma_d_.^2 ./ sqrt(sigma_m^2 + sigma_d_.^2));
    sigma_log = sqrt(log(1 + (sigma_m^2 ./ sigma_d_.^2)));
    sigma_hat = lognrnd(mu_log, sigma_log, [numel(uniqStimVals), nTrials]);
