function cv_result = NLLCrossValidate(data, initCond, K, nPerm, optParams)
...
resultsListCov = cell(nPerm, K); % cov
resultsListInd = cell(nPerm, K); % ind
resultsListSS  = cell(nPerm, K); % ss
...
for h=1:nPerm %parfor
    ...
    for k = 1:K
        ...
        results = Optimize(data, initCond, "cov", trainIdx, optParams, "full");
        resultsListCov{h, k} = results;
        
        fprintf("Running ind ...");
        % Run multi-start optimization for ind model
        results = Optimize(data, initCond, "ind", trainIdx, optParams, "full");
        resultsListInd{h, k} = results;
        
        fprintf("Running singly stochastic ...");
        % Run multi-start optimization for singlyStochastic model
        results = Optimize(data, initCond, "singlyStochastic", trainIdx, optParams, "full");
        resultsListSS{h, k} = results;
...
Data.resultsListSS  = resultsListSS;
