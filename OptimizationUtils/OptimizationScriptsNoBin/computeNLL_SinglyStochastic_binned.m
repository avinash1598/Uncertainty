...
function nll = computeNLL_SinglyStochastic_binned(params, metaData, fitType, optimizationFlag)
...
param_sigma_meta      = params(nLevels + 1);
param_Cc              = params(nLevels + 2);
param_guessrate       = params(nLevels + 3);

if fitType == "full"
    param_ori_scale = params(nLevels + 4);
    param_bias      = params(nLevels + 5);
end
...
for i=1:nLevels
    ...
    if fitType == "full" 
        ...
        retData = getPDFs_SinglyStochastic(orientations, modelParams, true); % Seems like setting this to false makes things slow
