function result = Optimize(data, initCond, fltTrlIdx, optParams)
    ...
    metaData.stimulusEnergy       = data.stimulusEnergy;
    
    % Run multi-start optimization for cov model
    result = multiStartFit(grpOriErr, n_uncertainty_levels, metaData, nStarts, initCond);
    
end

%% Fucntions
% function to perform optimization (do with multistart option)
function results = multiStartFit(grpOriErr, n_uncertainty_levels, metaData, nStarts, initCond)
% model: additive, multiplicative

nParams = 2*n_uncertainty_levels + 6;
...
parfor itr = 1:nStarts
    ...
    while ~success
        try 
            ...
            param_sigma_ext       = std(grpOriErr, [], 2)';  %100*rand(1, n_uncertainty_levels); %
            param_b               = b; %rand(1, n_uncertainty_levels);
            param_scale           = rand;
            ...
            %ub(:)   = 1000; % Dummy ub for now : TODO: remvoe it later
            ub(end - 2) = 1; % Upper bound for last parameter i.e. guessrate
