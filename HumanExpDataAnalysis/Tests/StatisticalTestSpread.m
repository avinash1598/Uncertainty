...
for fIdx=1:numel(dataFileNames)
    ...
    for c=1:numel(uniqContrasts)
        for d=1:numel(uniqDurations)
            ...
            fltData = fltData(fltData.stimContrast == uniqContrasts(c) & fltData.stimDur == uniqDurations(d), :);
            if numel(fltData) == 0
                continue
            end
            ...
            if size(formattedData.uncertaintyVals, 1) == 2
                ...
                assertionChecks(formattedData, 'stimSpread')
                reorderedData = reorderThings(formattedData, 'stimSpread');
                assertionChecks(reorderedData, 'stimSpread')
...
ylabel("Count")
title(sprintf("Multiplicative/Additive test for stim spread \n pooled across all subjects"))
ylim([0 50])
