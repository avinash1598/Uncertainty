...
sigma_m          = 0.2;
Cc               = 0.5;
rho              = 0.6; % Controls covariance between internal and external noise
...
for level = 1:numel(sigma_e_levels)
    ...
    for i = 1:numel(orientations)
        ...
        % Internal noise is greater than external noise
        % This doesn't fully seem correct
        % TODO: make this into signal dependent noise

        % Correct rerminology would be - noise fluctuations covaries with
        % level of noise in the system itself (not really I think)
        % IT probably models if the other sources of internal sensory noise 
        % covaries with other noise (external or sensory noise)
        sigma_i = sqrt( 9*(sigma_e^2) ); % Internal and external noise covaries
        
        Sigma = [sigma_e^2, rho * sigma_e * sigma_i;
                 rho * sigma_e * sigma_i, sigma_i^2];
        
        % Sample the covaried internal and external noise samples
        noise = mvnrnd([0 0], Sigma, trials);
        sigma_e_samples = noise(:,1);
        sigma_i_samples = noise(:,2);
        
        % Adjust gain of internal noise samples
        gain = gamrnd(1./varGain, varGain, [trials 1]);
        sigma_i_modulated = gain.*sigma_i_samples;
        sigma_theta = sqrt(sigma_e_samples.^2 + sigma_i_modulated.^2);
...
%%
% Logical mask for valid (non-NaN) pairs
validIdx = ~isnan(xvals(:)) & ~isnan(someDummyArray(:));

% Compute Spearman correlation only on valid entries
[corr_val, pval] = corr(xvals(validIdx), someDummyArray(validIdx), 'Type', 'Spearman');

retData = doBootstrapTrendTest(sigma_e_levels, resp_HC, resp_LC, true)
