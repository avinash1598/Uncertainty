...
sigma_m          = 0.2;
Cc               = 0.5;
sigma_stim_spread = 0.19; % Simulating external noise
sigma_stim_mean   = 0.1;  % Mean variability affects the result but spread variance does not!
...
for level = 1:numel(sigma_e_levels)
    ...
    for i = 1:numel(orientations)
        ...
        % Variable percision
        sample_spreads = sigma_e + sigma_stim_spread .* randn(trials, 1);
        sample_means = theta_true + sigma_stim_mean .* randn(trials, 1);
        
        sigma_i_modulated = gamrnd(sample_spreads./varGain, varGain, [trials 1]);
        sigma_theta = sigma_i_modulated;
        theta_est = sample_means + sigma_theta .* randn(trials, 1);
        ...
        % Store
%         theta_true_all(level, i, :)         = sample_means; % Error wrt sample mean
