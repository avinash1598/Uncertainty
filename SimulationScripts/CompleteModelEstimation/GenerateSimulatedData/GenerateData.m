function data = GenerateData(modelParams, n_uncertainty_levels, modelType, orientations, ntrials_per_ori)
    ...
    sigma_meta     = modelParams(n_uncertainty_levels + 1);
    Cc             = modelParams(n_uncertainty_levels + 2);
    guessRate      = modelParams(n_uncertainty_levels + 3);
    ...
    if modelType == "reduced"
        ...
        bias(:)         = 0;
    elseif modelType == "full"
        sigma_ori_scale = modelParams(n_uncertainty_levels + 4);
        biasAmp         = modelParams(n_uncertainty_levels + 5);
        sigma_s_stim    = sigma_s' + (sigma_ori_scale.*sigma_s)'*(abs(sind(orientations - 90)));
        bias            = biasAmp*sind(2*orientations); 
    elseif modelType == "full_jumbo"
        sigma_a          = modelParams(n_uncertainty_levels + 4:2*n_uncertainty_levels + 3);
        biasAmp          = modelParams(2*n_uncertainty_levels + 5);
    ...
    for l=1:n_uncertainty_levels
        for i = 1:n_theta
            ...
            % Single stochastic - No double stochasticity
            sigma_m_stim = sigma_s_stim(l, i);
