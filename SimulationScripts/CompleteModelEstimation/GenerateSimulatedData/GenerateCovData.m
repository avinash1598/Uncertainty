function data = GenerateCovData(modelParams, n_uncertainty_levels, modelType, orientations, ntrials_per_ori)
    
    assert(modelType == "reduced" || modelType=="full" || modelType == "full_jumbo")
    
    if modelType == "reduced"
        assert(numel(modelParams) == 10);
    elseif modelType == "full"
        assert(numel(modelParams) == 12);
    elseif modelType == "full_jumbo"
        assert(numel(modelParams) == 17);
    end
    ...
    scale          = modelParams(n_uncertainty_levels + 1);
    sigma_meta     = modelParams(n_uncertainty_levels + 2);
    Cc             = modelParams(n_uncertainty_levels + 3);
    guessRate      = modelParams(n_uncertainty_levels + 4);
    ...
    if modelType == "reduced"
        ...
        bias(:)         = 0;
    elseif modelType == "full"
        sigma_ori_scale = modelParams(n_uncertainty_levels + 5);
        biasAmp         = modelParams(n_uncertainty_levels + 6);
        sigma_s_stim    = sigma_s' + (sigma_ori_scale.*sigma_s)'*(abs(sind(orientations - 90)));
        bias            = biasAmp*sind(2*orientations); 
    elseif modelType == "full_jumbo"
        sigma_a          = modelParams(n_uncertainty_levels + 5:2*n_uncertainty_levels + 4);
