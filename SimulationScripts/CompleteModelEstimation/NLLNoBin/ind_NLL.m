...
%% Optimize
optParams.nStarts = 1;
result = Optimize(data, initCond, "ind", [], optParams, "full");
...
trlNLLs = getNLLForEachTrial(data, initCond, "ind", "full", fitParams, []);
nll_binned_method = trlNLLs;

fprintf("\n\nNLL (fit): %.4f, \nNLL (fit, binned method): %.4f) \n\n", result.f(idx), sum(nll_binned_method));

%% Plot (Simulated data and then optimization fit)
errBins = -90:2:90;
% plotFitResultSim(data, modelParams, initCond, "ind", "full", errBins)
plotFitResultSim(data, fitParams, initCond, "ind", "full", errBins)
