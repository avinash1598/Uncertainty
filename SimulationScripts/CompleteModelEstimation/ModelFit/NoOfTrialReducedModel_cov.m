...
T = table([], [], [], ...
    ...
    'scale', 'sigma_meta', 'Cc', 'paramsFitLoss', 'curveFitLoss'});
...
for itr = 1:nItr
    ...
    while ~success
        ...
        try
            ...
            ntrials_per_ori  = 25;
            b                = linspace(1, 2.2, 8); % Choose b such that average noise level ranges from low to high (relative to internal noise level)
            ...
            save('modelContOriData_trialCntTest_cov.mat', "data")
            
            
            %% Start fitting
            modelData = load('modelContOriData_trialCntTest_cov.mat');
            ...
            save("ModelFitMetrics_cov_common_objective.mat", "dataToSave");
...
% loss = sum( lossHC + lossLC , 'all'); % + sum( ( param_sigma_s - targetStds ).^2 );
