...
niterations = 100;
ntrials_per_ori_list = linspace(50, 1000, 20);

unpairedTest = zeros(numel(ntrials_per_ori_list), 1);
...
for tidx=1:numel(ntrials_per_ori_list)
    ...
    for itr=1:niterations
        ...
        sigma_e_levels   = linspace(0.01, 2, 25);  % Varying external noise or contrast
        sigma_i          = 1;
        varGain          = 0.5;
        sigma_m          = 0.2;
        Cc               = 0.5;
        ...
        if (numel(x1) > 0) && (numel(x2) > 0) && (numel(x3) > 0) && (numel(x4) > 0)
            % Bootstrap test
            result = doBootstrapTest(x1, x2, x3, x4);
        ...
        tempUnpairedTest = tempUnpairedTest + (p < 0.05);
        
        % This isn't right. Check at each contrast level that HC and LC
        % are not NaN
        % Bootstrap trend test
        result = doBootstrapTrendTest(sigma_e_levels, resp_HC, resp_LC);
    ...
    unpairedTest(tidx) = tempUnpairedTest;
...
plot(ntrials_per_ori_list, unpairedTest)
