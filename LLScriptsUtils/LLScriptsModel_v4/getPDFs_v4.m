function [retData] = getPDFs_v4(modelParams, stimVals, optimizationFlag)
...
if ~isfield(modelParams, 'internalNoiseSamplesCnt')
    modelParams.internalNoiseSamplesCnt = 200; % Default value obtained from params check analysis
end
...
% sigma_ln                  = modelParams.scale; % sigma of log normal distribution
...
stimulusEnergy            = modelParams.stimulusEnergy;
...
sigma_int_stim  = b + a.*(abs(sind(sigma_m_shape1*stimOris - sigma_m_shape2)));

% TODO: later maybe add transduction parameter
...
scaleParam = scale;
shapeParams = sigma_int_stim.^2 ./ scaleParam;
...
var_int_noise_stim = gammaSamples; % Measurement noise

% This might be wrong
% % Try log normal distribution instead
% mu = log(sigma_int_stim.^2) - 0.5 * sigma_ln^2;
% lognSamples = zeros(numel(mu), internalNoiseSamplesCnt);
% 
% for i = 1:numel(mu)
%     lognSamples(i, :) = logninv(linspace(1/internalNoiseSamplesCnt, 1 - 1/internalNoiseSamplesCnt, internalNoiseSamplesCnt), ...
%         mu(i), sigma_ln);
% end
% var_int_noise_stim = lognSamples;

% Measurement noise (additive, multiplicative)
sigma_m_stim = sqrt( var_ext_noise + var_int_noise_stim );
...
% TODO: analytical solution for sensory noise
