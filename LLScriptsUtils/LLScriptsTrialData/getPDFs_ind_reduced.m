...
% sigma_m = sqrt( sigma_s'.^2 + (gammaSamples).^2 ); % Measurement noise - this is blunder (avoid this and understand this)
sigma_m = sqrt( sigma_s'.^2 + (gammaSamples) ); % Something new - this is the right approach
...
[pdf, pdfHC, pdfLC, sigma, sigmaHC, sigmaLC, mad_m, mad_m_HC, mad_m_LC] = getGaussianMixturePDF(rvOriErrs, ...
    sigma_m, pHC, pLC, guessRate, K, optimizationFlag);
...
retData.pHC  = mean(pHC);
retData.pLC  = mean(pLC);

% Expected sigma - LC and HC - combined across all orientations
% retData.E_sigma_m_HC = sqrt( sum( (sigma_m.^2).*pHC, 'all')/sum(pHC, 'all') ); % this is right
% retData.E_sigma_m_LC = sqrt( sum( (sigma_m.^2).*pLC, 'all')/sum(pLC, 'all') );
retData.E_sigma_m_HC = sigmaHC;
retData.E_sigma_m_LC = sigmaLC;

% Expected sigma for all perceptual reports
% retData.E_sigma_m = sqrt( sigma_s.^2 + scaleParam*shapeParam );
...
if ~optimizationFlag
    ...
    if ~isnan(F(end))
        ...
        %[Funiq, idx] = unique(F);
        %xuniq = x(idx);
        median_val = 0; %interp1(Funiq, xuniq, 0.5); % median_val = 0
