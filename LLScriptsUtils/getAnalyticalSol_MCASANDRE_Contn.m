function [retData] = getAnalyticalSol_MCASANDRE_Contn(modelParams)
...
sigma_m_stim = sqrt( sigma_s_stim'.^2 + (sigma_si'.*gammaSamples).^2 ); % Measurement noise
...
% Expecyted sigma for each stimulus orientation - aggreagted by HC and LC 
retData.E_sigma_m_stim = sqrt( sigma_s_stim.^2 + sigma_si.^2 * scaleParam.^2 * (shapeParam * (shapeParam + 1)) );
